...
import MemoryCard from "./MemoryCard"; // Preview card
...
const estimateDateFromAge = (ageStr) => {
  const birthYear = 1998; // hardcoded for now
  const match = ageStr.match(/(\d+)/);
  if (match) {
    const estYear = birthYear + parseInt(match[1]);
...
const MemoryForm = ({ memoryToEdit = null, onClose }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const processedForm = {
      ...formData,
      date: formData.date || estimateDateFromAge(formData.age || "")
    };
    if (memoryToEdit) {
